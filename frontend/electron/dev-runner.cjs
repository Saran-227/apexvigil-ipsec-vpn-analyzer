...
async function run() {
  console.log('[DevRunner] Checking Vite dev server on :5173...')
  let viteProcess = null
  const alreadyUp = await isPortOpen('127.0.0.1', 5173)

  if (!alreadyUp) {
    console.log('[DevRunner] Launching Vite Dev Server...')
    ...
    console.log('[DevRunner] Vite dev server already active on :5173.')
  }

  console.log('[DevRunner] Launching Electron desktop window...')
  ...
  electronProcess.on('close', (code) => {
    console.log(`[DevRunner] Electron window closed (exit code ${code}). Cleaning up...`)
    if (viteProcess) {
